...
/* Statistics functionality code starts here */
...
function query_last_week(){
  return "SELECT sum( amount ) AS value
FROM wp_xf_data
WHERE date > DATE_SUB( CURDATE( ) , INTERVAL 7
DAY )
AND result =1";
}
...
function query_since_beginning(){
  return "select SUM(amount) as value from wp_xf_data where result=1";
}

function query_since_beginning_referals(){
  return "select SUM(amount) as value from wp_xf_data_referals where result=1";
}
